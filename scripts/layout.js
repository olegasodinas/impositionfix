...
*/
	...
	window.updatePageTransformsOnly = function(){
		...
		const gSX = window.__currentScaleX || 1;
		const gSY = window.__currentScaleY || 1;
		const gOffX = window.__offsetX || 0;
		const gOffY = window.__offsetY || 0;
		...
		for(let i=0; i<previewEls.length; i++){
			...
			const sx = (typeof slotT.scaleX === 'number') ? slotT.scaleX : ((typeof pageT.scaleX === 'number') ? pageT.scaleX : gSX);
			const sy = (typeof slotT.scaleY === 'number') ? slotT.scaleY : ((typeof pageT.scaleY === 'number') ? pageT.scaleY : gSY);
			const offX = (typeof slotT.offsetX === 'number') ? slotT.offsetX : ((typeof pageT.offsetX === 'number') ? pageT.offsetX : gOffX);
			const offY = (typeof slotT.offsetY === 'number') ? slotT.offsetY : ((typeof pageT.offsetY === 'number') ? pageT.offsetY : gOffY);
	...
	window.adjustContentSkew = function(x, y, pageNums){
		if(window.__selectedSlots && window.__selectedSlots.length > 0){
			window.__selectedSlots.forEach(i => {
				if(!window.__slotTransforms[i]) window.__slotTransforms[i] = {};
				if(typeof x === 'number') window.__slotTransforms[i].skewX = x;
				if(typeof y === 'number') window.__slotTransforms[i].skewY = y;
			});
			window.updatePageTransformsOnly();
			return;
		}
		...
		if(typeof x !== 'number') x = (firstPage && window.__pageTransforms[firstPage] ? window.__pageTransforms[firstPage].skewX : window.__skewX) || 0;
		if(typeof y !== 'number') y = (firstPage && window.__pageTransforms[firstPage] ? window.__pageTransforms[firstPage].skewY : window.__skewY) || 0;

		if(pages.length > 0){
			pages.forEach(p => {
				if(!window.__pageTransforms[p]) window.__pageTransforms[p] = {};
				window.__pageTransforms[p].skewX = x;
				window.__pageTransforms[p].skewY = y;
			});
		} else {
			window.__skewX = x;
			window.__skewY = y;
		...
		if(typeof skewXInput !== 'undefined' && skewXInput && document.activeElement !== skewXInput) skewXInput.value = x;
		if(typeof skewYInput !== 'undefined' && skewYInput && document.activeElement !== skewYInput) skewYInput.value = y;
	...
	window.calculatePageFit = function(w, h, availW, availH, rotation){
		let r = (rotation || 0) % 360;
		if(r < 0) r += 360;

		const useSmartFit = !!window.__preferUpscaleNotRotate;
		let scale;

		if(useSmartFit){
			// Calculate bounding box of rotated page to ensure it fits at any angle
		...
		const treatAsRotated = isVertical && useSmartFit;

		return { scale, rotation: r, treatAsRotated };
	};
	...
	window.fitSheetsToWorkspace = function(forceScale){
		...
		const availW = workspace.clientWidth - padLeft - padRight;
		const availH = workspace.clientHeight - padTop - padBottom;
